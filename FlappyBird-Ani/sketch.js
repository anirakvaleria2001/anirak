...
function preload() {
  ...
  imagenPersonaje = loadImage('./images/hada.png');
  imagenPared = loadImage('./images/tronco.png');
  musicaRecord = loadSound('./sounds/aplauso.wav');
  musicaFondo = loadSound('./sounds/musicafondo.mp3');
  musicaEspera = loadSound('./sounds/musicaEspera.mp3');
  fuente = loadFont('./images/fairy.ttf');
}
  ...
  if (estado === 1) {
    ...
    for (let i = 0; i < wallX.length; i++) {
      ...
      if (
        posY > height ||
        posY < 0 ||
        (abs(wallX[i] - 300) < 60 && abs(posY - wallY[i]) > 100)
      ) {
        estado = 0;
        musicaFondo.stop();
        cursor();
    ...
    //Significa que estamos en la pantalla de inicio
    background(0);
    imageMode(CORNER);
